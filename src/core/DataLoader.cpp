...
void DataLoader::saveSettings(const std::string& path, const data::SettingsData& settings) {
    ...
    j["gameSpeed"] = static_cast<nlohmann::json::number_integer_t>(settings.gameSpeed);
...
void DataLoader::saveProgress(const std::string& path, const data::SaveData& save) {
    nlohmann::json j;
    j["lastUnlockedLevel"] = static_cast<nlohmann::json::number_integer_t>(save.lastUnlockedLevel);
    j["coins"] = static_cast<nlohmann::json::number_integer_t>(save.coins);
    nlohmann::json::array_t badges;
    for (const auto& badge : save.badges) {
        badges.emplace_back(badge);
    }
    j["badges"] = badges;
    nlohmann::json::array_t towers;
    for (const auto& towerId : save.unlockedTowers) {
        towers.emplace_back(towerId);
    }
    j["unlockedTowers"] = towers;
    nlohmann::json::array_t completed;
    for (const auto& level : save.completedLevels) {
        completed.emplace_back(level);
    }
    j["completedLevels"] = completed;
