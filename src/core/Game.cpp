...
void Game::update(float dt) {
    if (m_state == GameState::Gameplay && !m_paused) {
        ...
        if (!m_pendingSpawns.empty()) {
            ...
            auto waveListIt = m_database.waves.find(m_currentLevelId);
            if (waveListIt != m_database.waves.end() &&
                m_waveIndex >= static_cast<int>(waveListIt->second.waves.size())) {
...
void Game::startLevel(const std::string& id) {
    ...
    if (towerIt != m_database.towers.end()) {
        for (const auto& cell : m_currentLevel.definition.buildable) {
            ...
            if (m_coins < towerIt->second.cost) break;
