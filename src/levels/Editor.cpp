...
void LevelEditor::exportJson(const std::string& path) const {
    nlohmann::json level;
    level["width"] = static_cast<nlohmann::json::number_integer_t>(m_width);
    level["height"] = static_cast<nlohmann::json::number_integer_t>(m_height);
    level["tileSize"] = static_cast<nlohmann::json::number_integer_t>(m_tileSize);
    nlohmann::json::array_t data;
    data.reserve(m_data.size());
    for (int value : m_data) {
        data.emplace_back(static_cast<nlohmann::json::number_integer_t>(value));
    }
    level["data"] = data;
